          ...
          <Accordion>
            ...
            <AccordionDetails>
              {results.functions && results.functions.length > 0 ? (
                <Stack spacing={2}>
                  {results.functions.map((func: any, index: number) => (
                      ...
                      <CardHeader
                        title={
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            <Chip 
                              label={`${func.function_type || 'user'}`} 
                              size="small" 
                              color={func.function_type === 'library' ? 'secondary' : 'primary'}
                            />
                          </Box>
                        }
                        subheader={
                          <Box>
                            <Typography variant="body2" color="text.secondary">
                              Address: <code>{func.address}</code> • Size: {func.size} bytes
                            </Typography>
                            {func.calls_to && func.calls_to.length > 0 && (
                              <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                                Calls: {func.calls_to.slice(0, 5).join(', ')}
                                {func.calls_to.length > 5 && ` ... (+${func.calls_to.length - 5} more)`}
                              </Typography>
                            )}
                          </Box>
                        }
          ...
          {!results.llm_translations ? (
            ...
            <>
              <Alert severity='success' sx={{ mb: 2 }}>
                <Typography variant='body2'>
                  <strong>LLM Provider:</strong> {results.llm_translations.provider || 'Unknown'} • 
                  <strong> Translation Time:</strong> {results.llm_translations.translation_time ? new Date(results.llm_translations.translation_time).toLocaleString() : 'Unknown'}
                </Typography>
              </Alert>
              
              {results.llm_translations.functions && results.llm_translations.functions.length > 0 && (
                <Box sx={{ mb: 2 }}>
                  <Typography variant='h6' gutterBottom>
                    Function Analysis ({results.llm_translations.functions.length} functions)
                  </Typography>
                  {results.llm_translations.functions.map((func: any, index: number) => (
                    <Accordion key={index} sx={{ mb: 1 }}>
                      <AccordionSummary expandIcon={<ExpandIcon />}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                          <Typography variant='h6'>
                            {func.function_name || `Function ${index + 1}`}
                          </Typography>
                          {func.confidence && (
                            <Chip 
                              label={`${Math.round(func.confidence * 100)}% confidence`} 
                              size='small'
                              color={func.confidence > 0.8 ? 'success' : func.confidence > 0.6 ? 'warning' : 'error'}
                            />
                          )}
                        </Box>
                      </AccordionSummary>
